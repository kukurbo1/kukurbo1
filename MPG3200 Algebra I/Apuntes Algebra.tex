...
Sea $G$ un grupo. ¿Cómo probar que $F_n \embbed G$? Hay una técnica bonita que permite hacer eso:
...
\begin{theorem}
  $F_2 \embbed \SL_2(\Z)$. Más precisamente,
...
\begin{exercise}
  Pruebe que $F_n \embbed F_2$ para $n \geq 2$.

  \emph{Hint:} Consideremos el conmutador $[F_2,F_2]$. Pruebe que $[F_2,F_2] \simeq F_\oo$, luego
  \[F_n \embbed F_\oo \simeq [F_2,F_2] \leqslant F_2.\]
\end{exercise}
...
\begin{obs}
  ...
  Si $\abs G$ no es potencia de $p$, entonces existe $q \neq p$ primo tal que $q \mid \abs G$. Por el Teorema de Cauchy, $\zmz q \embbed G$, y entonces $\exists a \in G$ tal que $\ord(a) = q$. $\to\gets$
\end{obs}
...
\end{obs}
\begin{proof}
  Supongamos que $p_i \mid \abs{E_d(G)}$. Entonces $\zmz{p_i} \embbed E_d(G)$. Ñuego $\exists g \in E_d(G) \setminus \{1\}$ tal que $g^{p_i} =1$.
