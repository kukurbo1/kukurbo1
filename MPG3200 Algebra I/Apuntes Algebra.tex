...
\begin{proof}
  ...
  \[ \ker(M) = \set{k \in \znz n \olon m_k = \id} = \{1\}. \]
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item $(A,\c)$ es semigrupo, %% definir semigrupo
...
\begin{example}
  ...
  \[ \Q(\xi) = \{a + b \xi \in \C \colon a,b \in \Q\}. \]
  Esto es un anillo.
\end{example}
...
\begin{definition}
  Si $\vphi \colon A \to B$ es homomorfismo de anillos biyectivo, se dice \wea{isomorfismo de anillos}. %% escribir las definiciones de epimorfismo, endomorfismo, monomorfismo.
\end{definition}
...
\begin{example}
  Sea $R$ un PID y sea $a \in R$. $a$ es irreducible $\iff (a)$ es maximal.
\end{example} %% demostrar

\begin{corollary}
  Si $R$ es PID, entonces son equivalentes: 
  \begin{enumerate}[(1)]
