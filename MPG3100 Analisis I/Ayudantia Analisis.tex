...
\newpage
\section{Ayudantía 6. Lunes 27 de Abril}

\subsection{Problema 1}
Sea $(X,\M,\mu)$ espacio de medida, y sea 
\[ \Ncal = \{N \in \M \colon \mu(N) = 0\}. \]
Definimos 
\[ \bar\M = \{E \cup F \colon E \in \M \land \exists N (F \subseteq N \land N \in \Ncal)\}. \]
Mostrar que $\bar\M$ es \( \sigma \)-álgebra y que existe una única extensión $\bar\mu$ de $\mu$ a una medida completa.
\begin{proof}
  El Fer mandará la solución eventualmente.
\end{proof}


\newpage
\subsection{Problema 2}
En la guía 4, tendrá que si $\vphi \colon \Ccal \to \R$ es una función de $\Ccal \subseteq \P(X)$, entonces $\mu^* \colon \P(X) \to [0,\oo]$ definida por 
\[ \mu^*(S) = \inf\set{\sum_{i=1}^\oo \phi(T_i) \colon S \subseteq \bigcup_{i=1}^\oo T_i, T_i \in \Ccal} \]
es medida exterior. Considere ahora $(X,\M_{\mu^*},\mu)$ el espacio de medida restringida a los medibles de $\mu^*$ (tomando $\mu = \mu^*|_{\M_{\mu^*}}$).
Sea $E \subseteq X$ tal que $\mu^*(E) < \oo$. Muestre que existe $A \in \Ccal_{\sigma\delta}$ (intersección numerable de elementos de $\Ccal_\sigma$) tal que 
\[ E \subseteq A \quad\land\quad \mu^*(E) = \mu^*(A). \]
Si además $E \in \M_{\mu^*}$ y $\Ccal \subseteq \M_{\mu^*}$, muestre que $A \in \M_{\mu^*}$ y $\mu(A \setminus E) = 0$.

\begin{proof}
  Para $n \in \N$, existe $\{C_i^{(n)}\} \subseteq \Ccal$ tal que $F \subseteq \bigcup_{i=1}^\oo C_i^{(n)} \eqqcolon A_n$ y 
  \[ \sum_{i=1}^\oo \vphi(C_i^{(n)}) \leq \mu^*(E) + \frac{1}{n}. \]
  Afirmamos que $A = \bigcap_{n=1}^\oo A_n$ cumple lo pedido.
  Notemos que 
  \[ \mu^*(A_n) = \sum_{i=1}^\oo \vphi(C_i) = \sum_{i=1}^\oo \mu^*(C_i) \geq \mu^*\br{\bigcup_{i=1}^\oo C_i} \geq \mu^*(E) \]
  y juntando esto con lo anterior, tenemos 
  \[ \mu^*(E) \leq \mu^*(A_n) \leq \mu^*(E) + \frac1n. \]
  Además 
  \[ \mu^*(E) \leq \mu^*(A) \leq \mu^*(A_n) \leq \mu^*(E) + \frac{1}{n}. \]
  Con $n \to \oo$ obtenemos $\mu^*(E) = \mu^*(A)$.

  Por último, si $E \in \M_{\mu^*}$ y $\Ccal \subseteq \M_{\mu^*}$, entonces $A$ es medible al ser intersección de unión de medibles. Luego
  \[ \mu(A \setminus E) = \mu(A) - \mu(E) \]
  \[ \mu(A) = \mu(A \cap E) + \mu(A \cap E^c) = \mu(E) + \mu(A \cap E^c). \]
\end{proof}

\newpage
\subsection{Problema 3}
Sea $\{f_n \colon X \o \R\}_n$ una colección de funciones medibles. Muestre que 
\[ \{x \in X \colon \lim_{n\to\oo} f_n(x) \text{ existe}\} \]
es medible.
...
\begin{proof}
  ...
  \begin{align*}
      &\{x \in X \colon \lim_{n\to\oo} f_n(x) \text{ existe}\} \\
      = &\{x \in X \colon \{f_n(x)\}_n \text{ es de Cauchy}\} \\
      = &\{x \in X \colon \forall \e>0, \exists N \in \N, \forall m,n \geq N, \abs{f_n(x) - f_m(x)} < \e\} \\
      = &\bigcap_{n=1}^\oo \set{x \in X \colon \exists N \in \N, \forall m,n \geq N, \abs{f_n(x) - f_m(x)} < \frac{1}{n}} \\
      = &\bigcap_{n=1}^\oo \bigcup_{N \in \N} \set{x \in X \colon \forall n,m \geq N, \abs{f_n(x) - f_m(x)} < \rac{1}{n}} \\
      = &\bigcap_{n=1}^\oo \bigcup_{N \in \N} \bigcap_{m,n \geq N} \set{x \in X \colon \abs{f_n(x) - f_m(x)} < \frac{1}{n}} \\
      = &\underbrace{\bigcap_{n=1}^\oo \underbrace{\bigcup_{N \in \N} \underbrace{\bigcap_{m,n \geq N} \underbrace{\underbrace{\abs{f_n - f_m}^{-1}}_{medible}((-\oo,1/n))}_{medible}}_{medible}}_{medible}}_{medible}. \\
  \end{align*}
  Tenemos entonces que el conjunto es intersección numerable de unión numerable de intersección numerable de preimágenes de intervalos bajo funciones medibles, por lo que es medible.
\end{proof}

\newpage
\subsection{Problema 4}

Sea $X = A \cup B$, con $A,B \in \M$. Sea $f \colon X \to \R$. Muestre que $f$ es medible en $X$ si y solo si $f$ es medible en $A$ y en $B$.
\begin{proof}
  Sabemos que $f$ es medible en $X$ si y solo si $f^{-1}((a,\oo)) \in \M$ para todo $a \in \R$. Esto es si y solo si 
  \[ (f^{-1}((a,\oo)) \cap A) \cup (f^{-1}((a,\oo)) \cap B) \in \M, \quad \forall a \in \R. \]
  \begin{itemize}
    \item \boxed{\impliedby} Si $f|_A$ es medible y $f|_B$ también, entonces 
    \[ f^{-1}((a,\oo)) \cap A , f^{-1}((a,\oo)) \cap B \in \M \]
    y su unión $f^{-1}((a,\oo))$ también.

    \item \boxed{\implies} Tenemos que $f^{-1}((a,\oo)) \in \M$. Luego $f^{-1}((a,\oo)) \cap A \in \M$. Por lo tanto $f|_A$ es medible. Lo mismo para $f|_B$.
  \end{itemize}
\end{proof}

\newpage
\subsection{Problema 5}
El mapa de Gauss es $T \colon [0,1) \to [0,1)$ dado por 
\[ T(x) = \begin{cases}
  0, &x = 0 \\
  x - \lfloor x \rfloor , &x > 0
\end{cases} \]
Muestre que $T$ es medible.
\begin{proof}
  Notemos que 
  \[ X = [0,1) = \{0\} \cup \left( \frac{1}{2},1 \right) \cup \bigcup_{n=2}^\oo \left( \frac{1}{n+1}, \frac{1}{n} \right]. \]
  Vemos que 
  \begin{align*}
      T|_{\{0\}} &= 0 \\
      T|_{(1/2,1)} &= \frac{1}{x} - 1 \\
      T|_{(1/(n+1), 1/n]} &= \frac{1}{x} - n.
  \end{align*}
  Son todas continuas, y entonces son medibles. Por el problema 4 (versión numerable) $T$ es medible.
\end{proof}


\newpage
\section{Ayudantía 7. Lunes 4 de Mayo}

\subsection{Problema 1}
Sea $\ds X = \bigcup_{n=1}^\oo X_n$, con $X_n \subseteq X_{n+1}$. Sea $f \colon X \to [0,\oo]$ medible. Muestre que $f$ es integrable en $X$ si y solo si $\exists M>0$ tal que 
\[ \int_{X_n} f \d\mu \leq M, \quad \forall n \in \N. \]

\begin{proof}
  \ \begin{itemize}
    \item \boxed{\implies} Supongamos que $f$ es integrable en $X$. Tomamos $\ds M = \int f \d\mu$. Entonces $\forall n \in \N$ tenemos
    \[ f\chi_{X_n} \leq f \implies \int_{X_n} f \d\mu = \int f\chi_{X_n} \d\mu \leq \int f \d\mu = M. \]
    \item \boxed{\impliedby} Sea $F_N = f \chi_{X_N}$. Entonces, como $X_n \subseteq X_{n+1}$ y $\ds X = \bigcup_{n=1}^\oo X_n$, tenemos que $F_N \nearrow f$. De aquí, 
    \[ \int f \d\mu = \lim_{N\to\oo} \int f \chi_{X_n} \d\mu < M < \oo. \]
...
\newpage
\subsection{Problema 2}
Sea $f \colon X \to [0,\oo]$ integrable. Para todo $E \subset X$ definimos 
\[ \nu(E) = \int_E f \d\mu. \]
Muestre que: 
\begin{enumerate}[(a)]
  \item $\nu$ es medida.
  \item $\ds \int g \d\nu = \int fg \d\mu$ para todo $g \colon X \to [0,\oo]$ integrable.
\end{enumerate}
\begin{proof}
  \ 
  \begin{enumerate}[(a)]
    \item Primero notemos que $\ds \nu(\varnothing) = \int_\varnothing f \d\mu = 0$.
    
    Sean $(E_n)_{n=1}^\oo$ disjuntos medibles. Tenemos que 
    \begin{align*}
        \nu\br{\bigcup_{n=1}^\oo E_n}
        &= \int_{\bigcup_{n=1}^\oo E_n} f \d\mu \\
        &= \int f \chi_{\bigcup_{n=1}^\oo E_n} \d\mu \\
        &= \int \sum_{n=1}^\oo \chi_{E_n} \d\mu \\
        &= \int \sum_{n=1}^\oo f \chi_{E_n} \d\mu \\
        &= \int \lim_{N\to\oo} \sum_{n=1}^N f \chi_{E_n} \d\mu \\
        &= \lim_{N\to\oo} \int \sum_{n=1}^N f \chi_{E_n} \d\mu \quad \text{(TCM)} \\
        &= \lim_{N\to\oo} \sum_{n=1}^N \int f \chi_{E_n} \d\mu \\
        &= \sum_{n=1}^\oo \int f \chi_{E_n} \d\mu \\
        &= \sum_{n=1}^\oo \int_{E_n} f \d\mu \\
        &= \sum_{n=1}^\oo \nu(E_n).
    \end{align*}

    \item Si $g$ fuese simple, 
    \[ g = \sum_{n=1}^N c_n \chi_{E_n} \]
    con $\{E_n\}_{n=1}^N$ disjuntos de a pares. En este caso $g f$ es $\mu$-integrable. Luego 
    \begin{align*}
        \int g \d\nu &= \sum_{n=1}^N c_n \nu(E_n) \\
        &= \sum_{n=1}^N c_n \int_{E_n} f \d\mu \\
        &= \sum_{n=1}^N c_n \int f \chi_{E_n} \d\mu \\
        &= \sum_{n=1}^N \int c_n f \chi_{E_n} \d\mu \\
        &= \int \br{\sum_{n=1}^N c_n f \chi_{E_n}} \d\mu \\
        &= \int f \cdot \br{\sum_{n=1}^N c_n \chi_{E_n}} \d\mu \\
        &= \int fg \d\mu.
    \end{align*}
    En general, si $S_n \nearrow g$ con $S_n$ simples, por convergencia monótona 
    \[ \int g\d\nu = \lim_{n\to\oo} \int S_n \d\nu = \lim_{n\to\oo} \int f S_n \d\mu = \int fg \d\mu. \]
  \end{enumerate}
\end{proof}
...
Sea $X$ localmente compacto Hausdorff. Sea $\mu$ medida regular en $\B(X)$. Para $U \subseteq X$ y $f \in \Ccal_c(X)$ denotemos 
\[ f \prec U \iff (\mathrm{supp}(f) \subseteq U \ \land \ 0 \leq f \leq \Chi{U}) \]
Si $U$ es abierto en $X$, muestre que 
\[ \mu(U) = \sup\set{\int f\d\mu \colon f \in \Ccal_c(X) \land f \prec U}. \]

\begin{proof}
  Sea $I = \sup\set{\int f \d\mu \colon f \in \Ccal(X) \land f \prec U}$. Queremos mostrar que $\mu(U) = I$.

  Si $f \prec U$, entonces $$\mu(U) = \int \Chi U \d\mu \geq \int f \d\mu.$$
  Tomando supremo 
  \[ mu(U) \geq I. \]
  Como $\mu$ es regular, 
  \[ \mu(U) = \sup\set{\mu(K) \colon K \subseteq U \text{ compacto}}. \]
  Existen $\{K_n\}_{n=1}^\oo$ compactos con 
  \[ \mu(U) - \frac{1}{n} \leq \mu(K_n) \leq \mu(U). \]
  Como $K_n \subseteq U$, afirmamos que existe $V_n$ abierto de clausura compacta tal que 
  \[ K_n \subseteq V_n \subseteq \bar{V_n} \subseteq U. \]
  Asumiendo la afirmación, $\bar{V_n}$ es normal (compacto Hausdorff).
  Tenemos que $K_n$ y $\bar{V_n} \setminus V_n$ son cerrados en $\bar{V_n}$. Luego por Urysohn, existe $f_n \colon V_n \to [0,1]$ continua con $f_n(K_n) = \{1\}$ y $f_n(\bar{V_n} \setminus V_n) = \{0\}$.
  Definimos $F_n \colon X \to [0,1]$ dada por 
  \[ F_n(x) = \begin{cases}
    f_n(x), &x \in \bar{V_n} \\
    0, &x \in V_n^c.
  \end{cases} \]
  Es continua, dado que $f_n$ y 0 en $V_n^c$ son continuas e iguales en el cerrado $\bar{V_n} \setminus V_n$. Por construcción, $F_N \prec U$. Luego $\Chi{K_n} \leq F \leq \Chi{U}$, y entonces 
  \[ \int \Chi{K_n} \d\mu \leq \int F_n \d\mu \leq \int \Chi{U} \d\mu \]
  \[\implies \mu(U) - \frac{1}{n} \leq \mu(K_n) \leq \int F_n \d\mu \leq \mu(U).\]
  Luego 
  \[ \lim_{n\to\oo} \int F_n \d\mu = \mu(U). \]
\end{proof}

\newpage
\subsection{Problema 4}

Sea $(X,\M,\mu)$ un espacio de medida, y sea $f \colon X \to [0,\oo]$ medible.

\begin{enumerate}
  \item Muestre que si $f$ solo toma valores enteros no negativos o $\oo$, entonces 
  \[ \int f \d \mu = \sum_{n=1}^\oo \mu(\{x \colon f(x) \geq n\}). \]
  \item Muestre que si $f$ toma valores arbitrarios de $[0,\oo]$ y $\mu$ es finita, entonces la integrabilidad de $f$ es equivalente a la convergencia de 
  \[ \sum_{n=1}^\oo \mu(\{x \colon f(x) \geq n\}). \]
\end{enumerate}

\begin{proof}
  \ 
  \begin{enumerate}
    \item Definimos para $n \in \N$: 
    \begin{align*}
        F_n &= \{x \colon f(x) \geq n\} \\
        E_n &= \{x \colon f(x) = n\} \\
        f_n &= \min\{f,n\}.
    \end{align*}
    Entonces $f_n \nearrow f$. Definimos $F_n^N = \{x \in X \colon n \leq f(x) \leq N\}$. Entonces
    \begin{align*}
        \int f_N &= \int \sum_{n=1}^N \Chi{E_n} \d\mu \\
        &= \int \sum_{n=1}^N n \Chi{E_n} \d\mu \\
        &= \sum_{n=1}^N n \mu(E_n) \\ 
        &= \sum_{n=1}^N \sum_{k=n}^N \mu(E_k) \\
        &= \sum_{n=1}^N \mu\br{\bigcup_{k=n}^N E_k} \\
        &= \sum_{n=1}^N \mu \br{\bigcup_{k=n}^N \{x \colon f(x) = n\}} \\
        &= \sum_{n=1}^N \mu \br{\{x \colon n \leq f(x) \leq N\}} \\
        &= \sum_{n=1}^N \mu (F_n^N)
    \end{align*}
    En $\N$ tomamos la medida discreta $\nu$ tal que si $f \in \R^\N$ es una sucesión $a_n = f(n)$, entonces 
    \[ \int_X f \d\nu = \sum_{n\in\N} a_n. \]
    Sea $a^N \colon \N \to [0,\oo]$ dada por 
    \[ n \mapsto \begin{cases}
      \mu(F_n^N), &n \leq N \\
      0, &n > N.
    \end{cases} \]
    $a \colon \N \to [0,\oo]$, $n \mapsto \mu(F_n)$. Luego 
    \[ a^N(n) = \mu(F_n^N) \nearrow \mu(F_n) = a(n). \]
...
\newpage
\section{Ayudantía 8. Miércoles 12 de Mayo}

\subsection{Problema 1}
Si $(X,\M,\mu)$ es un espacio de medida, $\{f_n\}_{n\in\N}, f$ medibles y 
\[ \int_X \abs{f_n - f} \d\mu \to 0 \]
(i.e.: $f_n \xrightarrow{L^1} f)$, entonces muestre que $f_n \xrightarrow{\mu} f$.

\begin{proof}
  Consideremos
  \[ E_{n,\e} = \{x \in X \colon \abs{f_n(x) - f(x)} \geq \e\}. \]
  Queremos ver que $\lim_{n\to\oo} \mu(E_{n,\e}) = 0$ para todo $\e>0$.
  Notemos que 
  \begin{align*}
      \mu(E_{n,\e}) &= \int_{E_{n,\e}} \d\mu \\
      &= \frac1\e \int_{E_{n,\e}} \e \d\mu \\
      &\leq \frac1\e \int_{E_{n,\e}} \abs{f_n - f} \d\mu \\
      &\leq \frac1\e \int \abs{f_n - f} \d\mu \xrightarrow{n\to\oo} 0.
  \end{align*}
\end{proof}
...
Sea $(\Omega,\M,\mu)$ espacio de medida y $\{f_n \colon X \to \R\}_{n\geq1}$ una sucesión tal que existe $M>0$ que cumple 
\[ \int_X \abs{f_n}^2 \d\mu \leq M \]
para todo $n \in \N$. Muestre que $\ds \frac1n f_n \to 0$ a.e.

\begin{proof}
  Notemos que 
  \[ \int \abs{\frac{1}{n} f_n}^2 \d\mu = \frac{1}{n^2} \int \abs{f_n}^2 \d\mu \leq \frac{M}{n^2}. \]
  Definimos 
  \[ g_n = \sum_{n=1}^N \abs{\frac{1}{n} f_n}^2 \]
  Por convergencia monótona 
  \[ \lim_{n\to\oo} g_n = \int \sum_{n=1}^\oo \abs{\frac{1}{n} f_n}^2 \d\mu \eqqcolon g. \]
  Luego por convergencia dominada, tenemos que 
  \[ \sum_{n=1}^\oo \int \abs{\frac{1}{n} f_n}^2 \d\mu \leq \sum_{n=1}^\oo \frac{M}{n^2} < \oo. \]
  Luego $\ds \sum_{n=1}^\oo \abs{\frac{1}{n} f_n}^2$ es finita a.e., y entonces $\ds \frac{1}{n} f_n \xrightarrow{n\to\oo} 0$ a.e.
\end{proof}

\newpage
\subsection{Problema 3}
Sea $(X,\M,\mu)$ un espacio de medida finita. Considere $\{f_n\}_n$ medibles tal que $\norm{f_n}_2 = 1$ y $\abs{f_n(x)} \leq M < \oo$ a.e. para todo $n \in \N$.

Si $\{\b_n\}_n \in \R^\N$ es tal que $\sum_n \b_n f_n$ converge a.e., muestre que $\b_n \to 0$.

\begin{proof}
  Notemos que 
  \[ \b_n^2 = (\b_n \norm{f}_2)^2 = \int (\b_n f_n)^2 \d\mu. \]
  Sea $\delta > 0$. Por Egorov, existe $B \in \M$ tal que $\mu(B^c) < \delta$ y $\b_n f_n \to 0$ uniformemente en $B$, pues $\sum_n \b_n f_n$ converge a.e. Luego 
  \begin{align*}
      \b_n^2 &= \int (\b_n f_n)^2 \d\mu \\
      &= \int_B (\b_n f_n)^2 \d\mu + \int_{B^c} (\b_n f_n)^2 \d\mu \\
      &\leq \int_B (\b_n f_n)^2 \d\mu + \int_{B^c} \b_n^2 M^2 \d\mu \\
      &= \int_B (\b_n f_n)^2 \d\mu + \b_n^2 M^2 \int_{B^c} 1 \d\mu \\
      &= \int_B (\b_n f_n)^2 \d\mu + \b_n^2 M^2 \mu(B^c) \\
      &< \int_B (\b_n f_n)^2 \d\mu + \b_n^2 M^2 \delta \\
      \implies (1-M^2\delta)\b_n^2 &\leq \int_B (\b_n f_n)^2 \d\mu \\
      \implies \b_n^2 &\leq \frac{1}{1-M^2\delta} \int_B (\b_n f_n)^2 \d\mu \xrightarrow{n\to\oo} 0
  \end{align*}
...
Suponga $(X,\M,\mu)$ finito. Para $f,g \colon X \to \R$ medibles 
\[ \rho(f,g) = \int_X \frac{\abs{f-g}}{1 + \abs{f-g}} \d\mu. \]
Muestre que $\rho$ es una métrica en el espacio de funciones medibles, identificando funciones a.e.. Además $f_n \to f$ en la métrica $\rho$ si y solo si $f_n \xrightarrow{\mu} f$.

\begin{proof}
  Para ver que es métrica:
  \begin{itemize}
    \item Si $\rho(f,g) = 0$, entonces 
    \[ \rho(f,g) = 0 \iff \frac{\abs{f-g}}{1 + \abs{f-g}} = 0 \text{ a.e.} \iff \abs{f-g} = 0 \text{ a.e.} \iff f = g \text{ a.e.} \]
    \item $\rho(f,g) = \rho(g,f)$ claramente.
    \item Notemos que para $A,B,C >0$ con $A \leq B + C$, siempre tenemos 
    \[ \frac{A}{1+A} \leq \frac{B}{1+B} + \frac{C}{1+C}. \]
  \end{itemize}
  Veamos que $f_n \to f$ en $\rho$ si y solo si $f_n \xrightarrow{\mu} f$.
  \begin{itemize}
    \item \boxed{\impliedby} Supongamos que $f_n \xrightarrow{\mu} f$. Consideramos el conjunto
    \[ E_{n,\e} = \{x \in X \colon \abs{f_n(x) - f(x)} \geq \e\}. \]
    Como $f_n$ converge en medida a $f$, tenemos que 
    \[ \mu(E_{n,\e}) \xrightarrow{n\to\oo} 0. \]
    Luego 
    \begin{align*}
        \rho(f_n,f)
        &= \int \frac{\abs{f_n - f}}{1 + \abs{f_n - f}} \d\mu \\
        &= \int_{E_{n,\e}} \frac{\abs{f_n - f}}{1 + \abs{f_n - f}} \d\mu + \int_{E_{n,\e}^c} \frac{\abs{f_n - f}}{1 + \abs{f_n - f}} \d\mu \\
        &\leq \int_{E_{n,\e}} 1 \d\mu + \int_{E_{n,\e}^c} \frac{\e}{1 + \abs{f_n - f}} \d\mu \\
        &= \mu(E_{n,\e}) + \e \int_{E_{n,\e}^c} \frac{1}{1 + \abs{f_n - f}} \d\mu \\
        &\leq \mu(E_{n,\e}) + \e \int_{E_{n,\e}^c} 1 \d\mu \\
        &= \mu(E_{n,\e}) + \e \underbrace{\mu(X)}_{<\oo}
    \end{align*}
    Luego $\rho(f_n,f) \to 0$.

    \item \boxed{\implies} Supongamos que $\rho(f_n,f) \xrightarrow{n\to\oo} 0$. Luego para $\e>0$ tomamos $\ds \tilde\e = \frac{\e}{1-\e}$ y entonces
    \begin{align*}
      &\mu\set{x \colon \abs{f_n - f} \geq \tilde\e} \\
      &= \mu\set{x \colon \abs{f_n - f} \geq \frac{\e}{1-\e}} \\
      &= \mu\set{x \colon \abs{f_n - f} \geq \e + \e \abs{f_n - f}} \\
      &= \mu\set{x \colon \frac{\abs{f_n - f}}{1 + \abs{f_n - f}} \geq \e} \\
      &\leq \frac{1}{\e} \int \frac{\abs{f_n - f}}{1 + \abs{f_n - f}} \d\mu.
    \end{align*} %% arreglar detalles y concluir
  \end{itemize}
\end{proof}
...
A modo de resumen: ¿Qué implica qué? Considere también si alguna otra hipótesis permite deducir alguna de las implicancias (por ejemplo, $\mu(X) < \oo$).

\begin{enumerate}[(i)]
  \item $f_n \to f$ a.e.
  \item $f_n \xrightarrow{\mu} f$.
  \item $\ds \int_X \abs{f_n - f} \d\mu \to 0$
  \item $\ds \int_X f_n \d\mu \to \int_X f \d\mu$.
\end{enumerate}

\begin{proof}
  \ \begin{itemize}
    \item \boxed{(ii) \not\implies (i)} Consideremos $f_n = \Chi{[n,\oo)}$, tenemos que $f_n \to 0$ a.e., pero para $\e = 1/2 > 0$ tenemos 
    \[ \mu\{x \in X \colon \abs{f_n - f} \geq 1/2\} = \mu\{x \in X \colon \abs{\Chi{[n,\oo)}} \geq 1/2\} = \mu[n,\oo) = \oo \neq 0 \]
    y entonces no converge en medida.
    \item \boxed{(ii) \not\implies (iii)} Consideramos $f_n = n \Chi{[0,1/n]}$.
    \item 
  \end{itemize}
\end{proof}
